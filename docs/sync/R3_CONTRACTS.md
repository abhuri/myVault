...
| `C07` | invalid/ambiguous text encoding | any content change | ห้าม merge และใช้ `PreserveBothLocal` ค่ะ |
| `C08` | merge input exceeds bounds | any content change | ห้าม merge และใช้ `PreserveBothLocal` ค่ะ |
...
- Portable collision key ใช้ Unicode NFC, Unicode default case folding,
  canonical `/` separators และ platform-invalid-name checks ค่ะ Unicode version
  และ naming version ต้อง persist กับ operation ค่ะ
