...
- [ ] R1 is merged into `main`, post-merge checks pass, and the R2 branch starts
  from that merge commitค่ะ
- [ ] R2 scope, non-goals, Drive mutation allowlist, retry taxonomy, content
...
- [ ] Dependency and schema migration plans receive security/data-loss reviewค่ะ

## Gate 1 — Native authorization and remote boundary

- [ ] Desktop and Android request only the exact restricted full Drive scope
  required for Existing Vault transferค่ะ
- [ ] Scope upgrade requires explicit re-consent and re-verifies the exact
  account/root binding before transferค่ะ
- [ ] Credentials and bearer-like resumable session URIs never enter the
  WebView, SQLite, logs, serialized errors, or durable historyค่ะ
- [ ] Provider origins are pinned, redirects are rejected, response metadata is
  bounded, and streamed bodies have explicit byte limitsค่ะ
- [ ] Captured requests prove only allowlisted metadata/media GET, create or
  resumable-init POST, and resumable-session PUT operations occurค่ะ
- [ ] No DELETE, Trash, rename, move, permission mutation, generic request API,
  or existing-different-content update is reachable in R2ค่ะ
- [ ] Exact account, root ancestry, parent ID, file ID, operation marker, remote
  revision, size, and digest checks fail closed when mismatchedค่ะ

## Gate 2 — Durable transfer state

- [ ] Schema v2 migrates transactionally to the exact v3 schema without losing
  binding, cursor, queue, history, or remote metadata evidenceค่ะ
- [ ] Newer, negative, partial, malformed, or constraint-weakened schemas are
  preserved and rejected without automatic repairค่ะ
- [ ] Queue evidence contains direction, exact identities, expected revisions,
...
- [ ] Credentials, resumable session URLs, provider bodies, note bodies,
  attachment bodies, and ambient paths are absent from SQLiteค่ะ
- [ ] Claim, retry schedule, `AuthRequired`, `NeedsReconcile`, verified
  completion, base publication, tombstone, and history transitions are atomicค่ะ
- [ ] Restart converts unknown running work to `NeedsReconcile` and never blindly
  replays a side effectค่ะ

## Gate 3 — Private staging and guarded local publication

- [ ] Markdown, zero-byte files, Unicode paths, and binary attachments stream
...
- [ ] Download fault injection covers request start, mid-stream, staged fsync,
...
- [ ] Exact retries are idempotent and mismatched operation-ID reuse is rejectedค่ะ
- [ ] Stale local or remote revisions, concurrent session switches, duplicate
  paths, and restarted workers preserve both sides without silent overwriteค่ะ
- [ ] R1 read-only behavior, local Vault safety, recovery, mutations, snapshots,
  auth, and frontend suites remain greenค่ะ

## Gate 6 — Offline quality and security

- [ ] `pnpm quality:r2:offline` passes frontend typecheck/tests/build plus Rust
  format, strict Clippy, unit, integration, migration, fault, and doc testsค่ะ
- [ ] Quality CI covers app-service, core, private-fs, snapshots, sync-engine,
  drive, transfer, desktop-auth, Google auth, private-root, vault-saf, and Tauriค่ะ
- [ ] Static and captured-request audits prove the exact R2 method/endpoint
  allowlist and absence of DELETE/Trash/rename/move/permission mutationค่ะ
- [ ] Secret/content/path audits find no token, session URI, provider body,
  content body, or ambient Vault path in frontend DTOs, logs, or SQLiteค่ะ
- [ ] Production dependency trees contain no `drive-sync-spike` and dependency
  review finds no unresolved high-risk additionค่ะ
- [ ] No unresolved P0/P1, data-loss, token-leak, or silent-overwrite finding
  remainsค่ะ
