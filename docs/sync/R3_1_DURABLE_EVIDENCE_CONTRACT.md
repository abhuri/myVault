...
R3.1 durable evidence ledger ถูกเพิ่มครั้งแรกใน historical `SCHEMA_VERSION = 4`
ค่ะ Current source ที่ `main@538fb72` ใช้ `SCHEMA_VERSION = 5` หลัง R3.3 เพิ่ม
exact base byte length โดยไม่เปลี่ยน immutable mutation/evidence/cursor semantics
ของ ledger ที่เริ่มใน v4 ค่ะ

Historical fresh database ของ R3.1 สร้าง v4 โดยตรง และ existing exact v3 database
migrate ภายใน SQLite transaction เดียวหลัง `quick_check` กับ exact v3 schema
validation ผ่านค่ะ สำหรับ exact v3 R3.1 ledger เส้นทางปัจจุบันคือ
`v3 → v4 → v5` ค่ะ Current source ยังรองรับ fresh v0 และ exact v1/v2 migration
paths ตาม source contract เดิมค่ะ Negative, newer-than-v5 หรือ structurally
non-exact schema ต้องถูก preserve และ reject ก่อนเขียน DDL ใด ๆ ค่ะ

หัวข้อ 4–9 freeze durable ledger semantics ที่เริ่มใน v4 และยังต้องถูก preserve
ใน current v5 ค่ะ SQL statement order และ Rust helper decomposition เป็น
implementation detail แต่ table/field semantics, constraints และ transition rules
ในเอกสารนี้ยังมีผลค่ะ R3.5 ต้องเริ่ม schema change จาก exact v5 และห้ามข้าม
หรือ reinterpret v3-to-v4/v4-to-v5 migration evidence ค่ะ
...
### 9.1 R3.4/R3.5 controlled local-identity handoff

Typed dependency ledger ที่เริ่มใน v4 และคงอยู่ใน current v5 รวมถึง
`FileRevision` evidence ปัจจุบันไม่ใช่ durable proof
ของ exact local object identity, atomic no-replace replacement หรือ final outcome
across restart ค่ะ R3.4 จึงไม่มี authority ที่จะ reinterpret evidence เดิมหรือเพิ่ม
executor เพื่ออ้าง proof เหล่านี้ค่ะ

หาก R3.5 ต้องเพิ่ม identity field, journal version, recovery transition,
idempotency marker หรือ echo/replay binding ต้องเสนอ controlled schema/journal/recovery
change ต่อ Sol High ก่อน source write ค่ะ Change ต้องรักษา immutable intent/event/
evidence binding, reject `legacy_v3` cursor advancement และ map ambiguous outcome เป็น
`WriteOutcomeUnknown` หรือ `NeedsReconcile` จนมี exact reconciliation proof ค่ะ

R3.5 prerequisite ผ่านแล้วจึงสามารถส่ง typed capability/evidence ที่ audit ได้กลับไป
ให้ R3.4 completion matrix ค่ะ การมี future plan หรือ watcher hint ไม่ถือเป็น
verification event และไม่อนุญาต cursor advancement ค่ะ
...
ณ เวลาตัดสิน Step 4 นั้น v4 ยังไม่อยู่บน canonical `main` และไม่มี released v4
database การแก้จึงคง `SCHEMA_VERSION = 4` และแก้ v3-to-v4 target schema โดยไม่
สร้าง v5 migration ค่ะ ภายหลัง R3.3 เพิ่ม controlled v4-to-v5 migration สำหรับ
exact base byte length ที่ `94cdffa` ค่ะ คำตัดสินทั้งสองช่วงไม่เปลี่ยน Option A,
provider boundary หรือ cursor safety ค่ะ
