...
- A Google token-endpoint client for authorization-code exchange and refresh,
  with redirects disabled, bounded responses, and redacted errors.
...
2. Open `authorization_url()` from native code with the system browser.
3. Call `wait_for_callback` on a worker thread with a short timeout.
4. Pass `TokenExchangeRequest` to `GoogleTokenClient` through the native
   `TokenExchanger` boundary.
5. Discover the provider-stable account ID using the returned access token,
   then save only the refresh token with `NativeTokenProvider`.
6. Request later access tokens through `NativeTokenProvider::fresh_access_token`.
7. On logout, revoke remotely when possible, then call the idempotent
