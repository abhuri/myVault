...
- R2 completed มี guarded content upload/download, durable
  ...
  และ post-merge Quality ผ่านแล้วค่ะ R3.1 durable evidence เป็น closure candidate
  ที่ Gate 1 ผ่าน locally แล้วค่ะ ส่วน R3.2 และภายหลังต้องมี approval แยกค่ะ
...
- R3 Safe Conflict Core planning pack แบ่งงานเป็น `R3.0 → R3.1 →
  ...
  R3.5 → R3.6 → R3.7` แล้วค่ะ R3.1 implementation complete locally และยังไม่มี
  provider mutation capability ค่ะ
...
1. Publish R3.1 closure candidate หลัง full validation, stage, commit และ push
   แล้วบันทึก exact canonical head/CI evidence ค่ะ
2. รักษา R3.1 frozen contract ใน
   [R3_1_DURABLE_EVIDENCE_CONTRACT.md](docs/sync/R3_1_DURABLE_EVIDENCE_CONTRACT.md)
   และห้ามเริ่ม R3.2, provider capability, live Drive หรือ external action ค่ะ
3. R3.2 classifier, R3.3 block enforcement, R3.4 และ provider capability ต้องมี
   approval แยกก่อนเริ่มค่ะ
...
- คุณโออนุมัติ `R3.1 closure plan` เมื่อ 2026-07-16 ค่ะ Closure audit พบ 3 P1 และ
  ...
  ก่อน publish ต้องรัน full validation, stage เฉพาะ R3.1, commit, push `main` และ
  บันทึก exact canonical head/CI result ค่ะ
